...
class SmartScheduler {
    ...
    public function generateSchedule($date = null) {
        ...
        try {
            ...
            foreach ($schedules as $schedule) {
                $total_volume += $schedule['total_volume'] ?? 0;
            }
            
            return [
                'success' => true,
                'message' => "Schedule generated successfully! $drivers_used drivers assigned to $total_requests requests.",
                'schedules' => $schedules,
                'date' => $date,
     ...
     */
    public function approveAllPendingRequests() {
        try {
            // Get columns available in requests table
            $schema_sql = "DESCRIBE requests";
            $schema_result = $this->conn->query($schema_sql);
            $available_columns = [];
            ...
            if (in_array('status', $available_columns)) {
                $update_sql .= "status = 'Approved'";
            } elseif (in_array('request_status', $available_columns)) {
                $update_sql .= "request_status = 'Approved'";
            } else {
                return [
                    'success' => false,
                    'error' => 'No status column found in requests table'
                ];
            }
            
            // Add WHERE clause for pending requests (case-sensitive enum values)
            if (in_array('status', $available_columns)) {
                $update_sql .= " WHERE status IN ('Pending', 'pending', '') OR status IS NULL";
            } elseif (in_array('request_status', $available_columns)) {
                $update_sql .= " WHERE request_status IN ('Pending', 'pending', '') OR request_status IS NULL";
            }
            
            // Execute the update
            $result = $this->conn->query($update_sql);
            
            if ($result) {
                $approved_count = $this->conn->affected_rows;
                
                return [
                    'success' => true,
                    'message' => "Successfully approved $approved_count pending requests",
                    'approved_count' => $approved_count
                ];
            } else {
                return [
                    'success' => false,
                    'error' => 'Failed to approve requests: ' . $this->conn->error
                ];
            }
            ...
            return [
                'success' => false,
                'error' => 'Error approving requests: ' . $e->getMessage()
            ];
        }
    }
    
    /**
     * Assign all approved requests to drivers for a specific date
     */
    public function assignAllToDrivers($date) {
        try {
            // Step 1: Get approved requests for the date
            $requests = $this->getApprovedRequests($date);
            if (empty($requests)) {
            ...
            // Step 3: Clear any existing schedules for this date
            $this->clearExistingSchedules($date);
            
            // Step 4: Optimize and assign requests to drivers
            $result = $this->optimizeAndAssign($requests, $drivers, $date);
            
            if (isset($result['success']) && $result['success']) {
                return [
                    'success' => true,
                    'message' => "Successfully assigned {$result['total_requests']} requests to {$result['drivers_used']} drivers",
                    'assigned_count' => $result['total_requests'],
                    'drivers_used' => $result['drivers_used'],
                    'date' => $date
                ];
            } else {
                // If result doesn't have 'success' key, it might be the schedule data directly
                if (is_array($result) && count($result) > 0) {
                    $total_requests = 0;
                    $drivers_used = count($result);
                    
                    foreach ($result as $schedule) {
                        $total_requests += isset($schedule['total_requests']) ? $schedule['total_requests'] : 0;
                    }
                    
                    return [
                        'success' => true,
                        'message' => "Successfully assigned {$total_requests} requests to {$drivers_used} drivers",
                        'assigned_count' => $total_requests,
                        'drivers_used' => $drivers_used,
                        'date' => $date
                    ];
                } else {
                    return $result; // Return as-is if it's an error response
                }
            }
            ...
            return [
                'success' => false,
                'error' => 'Error assigning requests: ' . $e->getMessage()
            ];
        }
    }
}
...
if ($action === 'view') {
    ...
    $response = $scheduler->approveAllPendingRequests();
} elseif ($action === 'assign_all') {
    $response = $scheduler->assignAllToDrivers($date);
} else {
