...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('../../backend/api/get_dashboard_metrics.php', { credentials: 'include' })
        .then(res => res.json())
        .then(data => {
    ...
    fetch('../../backend/api/get_recent_requests.php', { credentials: 'include' })
        .then(res => res.json())
        .then(data => {
            ...
            if (requests.length === 0) {
                requestsTable.innerHTML = '<tr><td colspan="5" style="text-align: center; color: #666;">No requests found</td></tr>';
            } else {
        ...
        .catch(error => {
            ...
            const requestsTable = document.getElementById('requests-table-body');
            requestsTable.innerHTML = '<tr><td colspan="5" style="text-align: center; color: #e74c3c;">Error loading requests</td></tr>';
        });

    // Fetch driver assignments
    fetch('../../backend/api/get_driver_assignments.php', { credentials: 'include' })
        .then(res => res.json())
        .then(assignments => {
            const assignmentsTable = document.getElementById('assignments-table-body');
            assignmentsTable.innerHTML = assignments.map(a => `
                <tr>
                    <td>${a.task_id}</td>
                    <td>${a.driver}</td>
                    <td>${a.request_id}</td>
    ...
    function displayPendingTasks(pendingRequests) {
        ...
        if (!pendingRequests || pendingRequests.length === 0) {
            pendingContainer.innerHTML = '<p style="grid-column: 1/-1; text-align: center; color: #666; padding: 20px;">No pending tasks available.</p>';
            return;
        }
        
        const tasksHTML = pendingRequests.map(request => `
            <div class="task-card">
                ...
                <div class="task-details">
                    <div class="task-detail-row">
                        ...
                        <span class="task-detail-value">${request.username || 'Unknown'}</span>
                    </div>
                    <div class="task-detail-row">
                        <span class="task-detail-label">Type:</span>
                        <span class="task-detail-value">${request.document || request.waste_type || 'Waste Collection'}</span>
                    </div>
                    <div class="task-detail-row">
                        <span class="task-detail-label">Date:</span>
                        <span class="task-detail-value">${request.created_at || 'N/A'}</span>
                    </div>
                </div>
                <div class="task-actions">
                    <button class="assign-task-btn" onclick="openDriverModal(${request.id})">
                        <i class="fas fa-user-plus"></i> Assign Driver
                    </button>
                    <button class="view-task-btn" onclick="viewTaskDetails(${request.id})">
    ...
    function displayAssignedTasks(assignedRequests) {
        ...
        if (!assignedRequests || assignedRequests.length === 0) {
            assignedContainer.innerHTML = '<p style="grid-column: 1/-1; text-align: center; color: #666; padding: 20px;">No assigned tasks available.</p>';
            return;
        }
        
        const tasksHTML = assignedRequests.map(request => `
            <div class="task-card">
                <div class="task-header">
                    <span class="task-id">Task #${request.id}</span>
    ...
    function openDriverModal(taskId) {
        ...
        fetch('../../backend/api/get_available_drivers.php', { credentials: 'include' })
            .then(res => res.json())
            .then(data => {
                if (data.success) {
    ...
    function displayDriversInModal() {
        ...
        if (!availableDrivers || availableDrivers.length === 0) {
            driverList.innerHTML = '<p style="text-align: center; color: #666; padding: 20px;">No available drivers found.</p>';
            return;
        }
        
        const driversHTML = availableDrivers.map(driver => {
            const capacityPercentage = (driver.current_load / driver.capacity) * 100;
                ...
                <div class="driver-item">
                    <div class="driver-info">
                        ...
                        <div class="driver-details">
                            ${driver.vehicle_type} • ${driver.phone || 'No phone'}
                        </div>
                    </div>
                    ...
                    <button class="select-driver-btn" onclick="assignTaskToDriver(${driver.id})">
                        Select
                    </button>
                </div>
            `;
    ...
    function viewTaskDetails(taskId) {
        ...
        alert(`Viewing details for task #${taskId}. This feature can be expanded to show full task details.`);
    }
    ...
    if (generateScheduleBtn) {
        generateScheduleBtn.addEventListener('click', function() {
            ...
            // Show loading state
            generateScheduleBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Generating...';
            generateScheduleBtn.disabled = true;
            
            // Generate schedule
            fetch(`../../backend/api/generate_schedule.php?date=${date}`, {
                credentials: 'include'
            })
            .then(res => res.json())
            .then(data => {
            ...
            .finally(() => {
                // Reset button state
                generateScheduleBtn.innerHTML = '<i class="fas fa-magic"></i> Generate Schedule';
    ...
    function displayScheduleResults(data) {
        ...
        // Display summary
        document.getElementById('total-requests-count').textContent = schedule.total_requests;
        document.getElementById('clusters-formed').textContent = schedule.clusters;
        document.getElementById('fuel-savings').textContent = schedule.estimated_fuel_savings.savings_percentage + '%';
    ...
    function displayRoutes(assignments) {
        ...
        const routesHTML = assignments.map((assignment, index) => {
            if (!assignment.route) return '';
            
            const stopsHTML = assignment.route.map((stop, stopIndex) => `
                <div class="stop-item">
                    <div class="stop-number">${stopIndex + 1}</div>
                    <div class="stop-details">
                        <div class="stop-address">${stop.address || 'Address not available'}</div>
                        <div class="stop-time">${stop.preferred_time || 'Time not specified'}</div>
                    </div>
                </div>
            `).join('');
                ...
                <div class="route-card">
                    <div class="route-header">
                        <div class="route-title">
                            ${assignment.driver_name ? `Route ${index + 1} - ${assignment.driver_name}` : `Route ${index + 1} - Unassigned`}
                        </div>
                        <div class="route-stats">
                            <span>${assignment.total_distance || 0} km</span>
                            <span>${assignment.estimated_duration || 0} min</span>
    ...
    function displayRecommendations(recommendations) {
        ...
        if (!recommendations || recommendations.length === 0) {
            recommendationsList.innerHTML = '<li>No specific recommendations for this schedule.</li>';
        } else {
